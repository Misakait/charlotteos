...
pub unsafe fn enable_early_mmu() {
    ...
    let kernel_phys_addr = unsafe { &_skernel as *const _ as usize };

    // 向下对齐到 1GB 边界，得到当前平台的纯粹 RAM 基址 (比如 0x80000000)
    let phy_base = kernel_phys_addr & !((1 << L2_INDEX_SHIFT) - 1);

    // 计算高半区基址
    let virt_base = PHYS_VIRT_OFFSET.wrapping_add(phy_base);

    // 获取静态根页表 (BOOT_PT) 的物理地址
    let pt_pa = (&raw const BOOT_PT as usize);
...
pub fn switch_to_final_page_table() {
